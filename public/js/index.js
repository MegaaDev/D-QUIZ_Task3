...
if (document.querySelector(".create-quiz-button")) {
  document
    ...
    .addEventListener("click", (e) => {
      ...
      let tags_array = [];
      for (let j = 0; j < tags_original.length; j++) {
        if (!tags_original[j] == "") {
          tags_array.push(tags_original[j]);
        }
      }
...
if (document.querySelector(".exit-quiz")) {
  document.querySelector(".exit-quiz").addEventListener("click", (e) => {
    window.location.href = "/";
  });
}
  ...
  document
    ...
    .addEventListener("click", (e) => {
      ...
      for (let j = 0; j < 20; j++) {
        if (
          document
            .querySelectorAll(".tag-set")
            [j].classList.contains("select-tag")
        ) {
          let temp_tag = document.querySelectorAll(".tag-set")[j].textContent;
          user_tags.push(temp_tag);
        }
      }
      e.preventDefault();
...
if (document.querySelector(".tag-set ")) {
  for (let i = 0; i < 20; i++) {
    document.querySelectorAll(`.tag-set`)[i].addEventListener("click", () => {
      if (
        !document
          .querySelectorAll(`.tag-set`)
          [i].classList.contains("select-tag")
      ) {
        document.querySelectorAll(`.tag-set`)[i].classList.add("select-tag");
        if (
          document
            .querySelectorAll(`.tag-set`)
            [i].classList.contains("deselect-tag")
        ) {
          document
            .querySelectorAll(`.tag-set`)
            [i].classList.remove("deselect-tag");
        }
      } else {
        document.querySelectorAll(`.tag-set`)[i].classList.remove("select-tag");
        document.querySelectorAll(`.tag-set`)[i].classList.add("deselect-tag");
      }
    });
  }
}
...
let array_tags = [];
let tag_count = 0;
if (document.querySelector(".top-right-container-tags")) {
  document
    .querySelector(".top-right-container-tags")
    .addEventListener("click", () => {
      let tag = document.querySelector(".top-left-container-tags input").value;
      array_tags =
        Array[
          document
            .querySelector(".secret-array")
            .textContent.replace("[", "")
            .replace("]", "")
        ] || [];
      tags_original.push(tag);
      console.log(tags_original, "this");
      tag_count++;
      ...
      for (let i = 0; i < array_tags.length; i++) {
        ...
        let mega = `
      <p>${array_tags[i]}</p>
      <img class="cancelit" src="./images/close.png" alt="" />
    `;
        abcda.innerHTML = mega;
        abcda.className = `tag-snippet tag-snippet-${tag_count}`;
        document.querySelector(".bottom-container-tags").append(abcda);
      }

      tempFunction();

      document.querySelector(".top-left-container-tags input").value = "";
      document.querySelector(".secret-array").textContent = array_tags;
    });
  const tempFunction = () => {
    console.log(tags_original, "heloo");

    for (let j = 0; j < tags_original.length; j++) {
      console.log("yeah");

      document
        .querySelectorAll(".cancelit")
        [j].addEventListener("click", () => {
          console.log("clicked");

          tags_original[j] = "";
          document
            .querySelector(`.tag-snippet.tag-snippet-${j + 1}`)
            .classList.add("hideITpls");
        });
    }
  };
}
